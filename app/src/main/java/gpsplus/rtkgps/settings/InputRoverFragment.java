...
public class InputRoverFragment extends PreferenceFragmentCompat {
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        mProcessingOptions = ProcessingOptions1Fragment.readPrefs(requireActivity());
        ...
        Preference streamCommandAtStartupButton = findPreference(KEY_COMMANDS_AT_STARTUP_SHUTDOWN_BUTTON);
        if (streamCommandAtStartupButton != null) {
            streamCommandAtStartupButton.setOnPreferenceClickListener(preference -> {
        ...
        if (stationBtn != null) {
            if (stationPositionButtonDisabledCause() == 0) {
                stationBtn.setOnPreferenceClickListener(preference -> {
        ...
        ListPreference listPreferenceAntrennas = findPreference("antenna");
    ...
    protected void initPreferenceScreen() {
        ...
        setPreferencesFromResource(R.xml.input_stream_settings_rover, null);

        initStreamTypePref();

        formatPref = findPreference(KEY_FORMAT);
        if (formatPref != null) {
    ...
    protected void initStreamTypePref() {
        ...
        types = new ArrayList<>(Arrays.asList(INPUT_STREAM_TYPES));

        if (BluetoothAdapter.getDefaultAdapter() == null) {
            types.remove(StreamType.BLUETOOTH);
        }

        typePref = findPreference(KEY_TYPE);
        if (typePref != null) {
            typePref.setValues(types.toArray(new StreamType[0]));
            typePref.setDefaultValue(DEFAULT_STREAM_TYPE);
        }
    }

    protected int stationPositionButtonDisabledCause() {
        if (mProcessingOptions != null && mProcessingOptions.getPositioningMode().isFixed()) {
            return 0;
        }else {
    ...
    protected void refresh() {
        ...
        // Ensure mProcessingOptions is initialized
        if (mProcessingOptions == null && getActivity() != null) {
        ...
        typePref = findPreference(KEY_TYPE);
        formatPref = findPreference(KEY_FORMAT);
        receiverOptionPref = findPreference(KEY_RECEIVER_OPTION);
        ...
        antennaList = findPreference(KEY_ANTENNA);

        if (typePref != null && formatPref != null) {
        ...
        if (receiverOptionPref != null) {
            receiverOptionPref.setSummary(receiverOptionPref.getText());
        }
        if (settingsButtonPref != null && prefs != null) {
