...
public class UsbToRtklib {
    ...
    private final class LocalSocketThread extends RtklibLocalSocketThread {
        ...
        protected boolean onDataReceived(byte[] buffer, int offset, int count) {
            ...
            try {
                ...
                Log.e(TAG, "onDataReceived: " + e.getMessage(),null);
                return false;
            }
            return true;
        }
    ...
    private class UsbReceiver {
        ...
        public synchronized void start() {
            ...
            ContextCompat.registerReceiver(mContext, mUsbStateListener, f, ContextCompat.RECEIVER_EXPORTED);
        ...
        private UsbSerialController probeDevice(UsbDevice d) {
            ...
            try {
                Log.d(TAG, "A");
                return new UsbPl2303Controller(mUsbManager, d, this.mContext);
            }catch(UsbControllerException e) {
                Log.d(TAG, "probeDevice: A" + e.getMessage());
            }

            try {
                Log.d(TAG, "B");
                return new UsbAcmController(mUsbManager, d, this.mContext);
            }catch (UsbControllerException e) {
                Log.d(TAG, "probeDevice: B" + e.getMessage());
            }

            try {
                Log.d(TAG, "C");
                return new UsbFTDIController(mUsbManager, d, this.mContext);
            }catch (UsbControllerException e) {
                Log.d(TAG, "probeDevice: C" + e.getMessage());
            }
        ...
        private void requestPermission(UsbDevice d) {
            ...
            boolean hasPermission = mUsbManager.hasPermission(d);
            Log.d(TAG, "requestPermission: hasPermission=" + hasPermission);

            if (hasPermission) {
                Log.d(TAG, "requestPermission: Already have permission, calling onUsbPermissionGranted");
                onUsbPermissionGranted(d);
                return;
            }

            Log.d(TAG, "requestPermission: Requesting permission via PendingIntent");
            ...
            Log.d(TAG, "requestPermission: Permission request sent");
        }
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                switch (Objects.requireNonNull(action)) {
                    case ACTION_USB_DEVICE_ATTACHED:
                        ...
                        assert device != null;
                        Log.d(TAG, "USB_DEVICE_ATTACHED: " + device);
                        onUsbDeviceAttached(device);
                        break;
                    case UsbManager.ACTION_USB_DEVICE_DETACHED:
                        device = intent.getParcelableExtra(UsbManager.EXTRA_DEVICE);
                        assert device != null;
                        Log.d(TAG, "USB_DEVICE_DETACHED: " + device);
                    ...
                    case ACTION_USB_PERMISSION:
                        ...
                        Log.d(TAG, "USB_PERMISSION: device=" + device + ", granted=" + granted);
                        if (granted) {
                            Log.d(TAG, "USB_PERMISSION: Permission granted, calling onUsbPermissionGranted");
                            onUsbPermissionGranted(device);
                        } else {
                            Log.d(TAG, "USB_PERMISSION: Permission denied by user");
                        }
                        break;
                    default:
        ...
        private class UsbServiceThread extends Thread {
            ...
            private void connectLoop() throws CancelRequestedException {
                ...
                // If loop exits due to cancelRequested
                throwIfCancelRequested();
            }
