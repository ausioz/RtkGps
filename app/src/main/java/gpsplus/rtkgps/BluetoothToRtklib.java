...
public class BluetoothToRtklib {
    ...
    public void start(Context ctx) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S &&
            ...
            Log.e(TAG, "Permission not granted. Bluetooth thread not started.");
    ...
    private final class LocalSocketThread extends RtklibLocalSocketThread {
        ...
        protected boolean onDataReceived(byte[] buffer, int offset, int count) {
            ...
            try {
                ...
                Log.e(TAG, "onDataReceived: " + e.getMessage() );
                return false;
            }
            return true;
        }
    ...
    private class BluetoothServiceThread extends Thread {
            ...
            try {
                while (running) {
                    ...
                    if (rcvd > 0) {
                        try {
                            ...
                            Log.e(TAG, "transferDataLoop write error: " + e.getMessage());
                            running = false;
                        }
                    } else if (rcvd < 0) {
                ...
                synchronized (this) {
                    try {
                        ...
                        Log.e(TAG, "transferDataLoop socket close error: " + e1.getMessage());
                    }
        ...
        public void run() {
            ...
            try {
                ...
                Log.i(TAG, "END BluetoothToLocalSocket-BT");
            } catch (Exception e) {
                Log.e(TAG, "Unexpected exception in Bluetooth thread", e);
            } finally {
                mCallbacks.onStopped();
            }
        }
